...
class DelayBuffer:
    ...
    Args:
        min_lag: Minimum lag (inclusive). Must be >= 0.
        max_lag: Maximum lag (inclusive). Must be >= min_lag.
        batch_size: Number of parallel environments.
        device: Torch device for storage.
        per_env: If True, sample a separate lag per environment.
        hold_prob: Probability in [0.0, 1.0] to keep the previous lag.
        update_period: If > 0, refresh lags every N steps; if 0, consider updating every step.
        per_env_phase: If True and update_period > 0, stagger refresh steps across batch.
        generator: Optional RNG for sampling lags.
